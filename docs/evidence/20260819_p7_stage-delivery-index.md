# P7 阶段交付索引（截至 2026-08-21）

负责人：P7 刘彦晗  
分支：`feature/cloud-llm-p7`  
代码基线：`5801d19`；已合并最新 `master`，工作区仅保留未跟踪的 `docs/temp-evidence/`
...
| 云端单元测试 | `cloud-llm-service/tests/test_service.py` | 18 项通过 |
| 边缘 HYBRID 契约修复 | `edge-agent/src/main.py` | 已合入 `5801d19` |
| 边缘回归测试 | `edge-agent/tests/` | 当前本地回归 100/100 通过（详见 `20260822_p7_local_validation.md`） |
...
- PR #8 已合并关闭，仅作为早期 MQTT consumer 历史记录；当前 P7 修改应使用新 PR，base=`master`、compare=`feature/cloud-llm-p7`。
- 已归档原始日志包含正常和校验失败记录；重复 request、trace 不匹配、未知事件、发布失败/云端不可用仍需补充独立异常证据。
- `docs/temp-evidence/` 保留 P1 原始副本，仅用于临时审阅；本轮不加入 Git。其原始日志与当前 `docs/evidence/` 三份日志为重复文件。

## 联动清单

- P1：使用 `5801d19` 重跑 CLOUD/HYBRID、重复 response、trace 不匹配、超时回退和 SQLite 状态回写。
- P2：按 `20260818_p7_frontend_metrics_contract.md` 接收 `cloud_inference`、`cloud_latency_ms`、pending/timeout 字段，补一条前端可见链路证据。
- P6：按服务器 `IP:1884` 或 SSH 隧道提供环境；当前先用 mock 补真实 Broker 超时，后续部署 vLLM 后补 endpoint、健康检查和真实模型证据。
- P5：统一云端延迟、成功率、异常样本统计口径，纳入三路线评测。
- P3：更新验收矩阵，创建并 review 新 Draft PR（base=`master`、compare=`feature/cloud-llm-p7`），引用本目录证据。

## PR 与真实模型核对

- 当前本地 refs：`feature/cloud-llm-p7 @ 5801d19`，`master @ b76bce8`；PR #8 已关闭，当前 P7 修改待新 Draft PR review。
- 当前正式归档的 CLOUD/HYBRID 原始日志标记 `LLM_MODE=mock`，仓库内仍没有当前 `5801d19` 分支的真实 14B/vLLM e2e 原始证据；该证据需要现场运行 commit、endpoint、模型版本和日志路径后才能核对。
