...
from .database import (
    ...
    EdgeShiftHandover, EdgeAgentMessage,
)
from .mqtt_handler import MqttHandler
from .websocket_manager import WebSocketManager
from .schemas import (AckRequest, ModelDeployRequest, EnvControlRequest,
                      ShiftSummaryRequest, InjectionRequest,
                      EdgeHandoverRequest, EdgeAskRequest)
...
@app.post("/api/edge-agent/handover/generate")
def generate_edge_handover(body: EdgeHandoverRequest, db: Session = Depends(get_db)):
    ...
    if result.get("status") != "ok":
        raise HTTPException(status_code=502, detail=f"边端生成失败: {result.get('error', '未知错误')}")
...
@app.get("/api/edge-agent/handovers")
...
def ask_edge_agent(body: EdgeAskRequest, db: Session = Depends(get_db)):
    ...
    if result.get("timeout"):
        raise HTTPException(status_code=504, detail="边缘 Agent 响应超时，请稍后重试")
    if result.get("status") != "ok":
        raise HTTPException(status_code=502, detail=f"边端问答失败: {result.get('error', '未知错误')}")
...
@app.get("/api/edge-agent/messages")
