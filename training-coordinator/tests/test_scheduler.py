...
def test_sync_round_lifecycle():
    ...
    assert r.state == RoundState.RUNNING
    assert len(r.participants) == 2
    assert r.min_participants == 2
    ...
    print("✓ test_sync_round_lifecycle 通过")
...
def test_insufficient_participants():
    ...
    print("✓ test_insufficient_participants 通过")


def test_strategy_enum():
    """策略枚举值应覆盖三阶段"""
    assert Strategy.SYNC_FEDAVG.value == "sync_fedavg"
    assert Strategy.ASYNC_STALE.value == "async_stale"
    assert Strategy.ROBUST.value == "robust"
    print("✓ test_strategy_enum 通过")
...
def test_round_state_machine():
    ...
    print("✓ test_round_state_machine 通过")


if __name__ == "__main__":
    test_strategy_enum()
    test_sync_round_lifecycle()
    test_insufficient_participants()
    test_round_state_machine()
    print("\n全部测试通过 ✓")
